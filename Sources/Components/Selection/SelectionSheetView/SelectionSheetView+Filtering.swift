...
extension SelectionSheetView {
    /// Filters `nodes` against `query`, case-insensitively, preserving order.
    ///
    /// Leaves are kept when their title or subtitle matches. A parent is kept whole
    /// when its own title/subtitle matches; otherwise it is kept only if some child
    /// matches, pruned to those children. An empty or whitespace-only query returns
    /// every node unchanged.
    /// - Parameters:
