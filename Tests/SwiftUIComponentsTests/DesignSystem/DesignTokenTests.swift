...
func defaultSpacingUsesApprovedScale() {
    let spacing = DefaultDesignSpacing()
...
func defaultShapeAndInteractionTokens() {
    let radius = DefaultDesignRadius()
    let stroke = DefaultDesignStroke()
    let motion = DefaultDesignMotion()
...
func defaultThemeExposesTokenGroups() {
    let theme = DefaultDesignTheme(spacing: DefaultDesignSpacing(twoUnits: 18))
...
func typographySupportsCustomFamily() {
    _ = DefaultDesignTypography(fontFamily: "Avenir Next")
}
